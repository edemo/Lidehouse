...
//    debugAssert(Meteor.isServer, 'Email addresses of users are not sent to the clients');
  ...
  hasPermission(permissionName, doc = { communityId: getActiveCommunityId() }, notParcelScoped = false) {
    ...
    rolesWithThePermission.forEach(role => {
      ...
      activeMemberships.forEach(membership => {
        if (membership.role === role || (membership.role + '@parcel' === role && ((!parcelId && notParcelScoped) || parcelId === membership.parcelId))) {
          result = true;
        }
      });
    });
